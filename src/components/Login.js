...
const Login = () => {
  ...
  var CLIENT_SECRET = "476eab89f65af91c2725f06fbe19652d4ae7ba07"
  ...
  useEffect(() => {
    gapi.load("client:auth2", () => {
      gapi.auth2.init({ clientId: "826628327925-nbfka0c4usc5g0p6eg0dt4fnraa68vml.apps.googleusercontent.com" })
    })
  }, []);

  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post("http://localhost:7000/user/login", { email, password }).then((res) => {
      if (res.data.success) {
  ...
  const facebookLoginHandler = () => {
    window.location = "http://localhost:7000/auth/facebook";
  }
  ...
  const onFailure = response => console.error(response, "error response");
  return (
      ...
      <div className="relative z-10 flex flex-wrap -m-8">
        ...
        <div className="w-full md:w-1/2 p-8">
          <div className="p-4 py-16 flex flex-col justify-center bg-blueGray-100 h-full">
            <form className="md:max-w-lg mx-auto" onSubmit={handleSubmit}>
              <label className="block mb-4">
                <p className="mb-2 text-gray-900 font-semibold leading-normal">Email Address *</p>
                <input className="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:ring focus:ring-indigo-300"
                  id="signInInput1-1"
              ...
              <label className="block mb-5">
                ...
                <input className="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:ring focus:ring-indigo-300" id="signInInput1-2"
                  type="password"
                  placeholder="********"
                  value={password}
              ...
              <button className="mb-9 py-4 px-9 w-full text-white font-semibold border border-indigo-700 rounded-xl shadow-4xl focus:ring focus:ring-indigo-300 bg-indigo-600 hover:bg-indigo-700 transition ease-in-out duration-200" type="submit">Sign In</button>
              <p className="mb-5 text-sm text-gray-500 font-medium text-center">Or continue with</p>
              <div className="flex flex-wrap justify-center -m-2">
                <div className="w-auto p-2">
                
                  <GoogleLogin
                    clientId="826628327925-nbfka0c4usc5g0p6eg0dt4fnraa68vml.apps.googleusercontent.com"
                ...
                <div className="w-auto p-2">
                  <button className="flex items-center p-4 bg-white hover:bg-gray-50 border rounded-lg transition ease-in-out duration-200">
                    ...
                    <span className="font-semibold leading-normal" onClick={() => facebookLoginHandler()}>Sign in with Facebook</span>
                  </button>

                </div>
                <div className="w-auto flex items-center p-4 bg-white hover:bg-gray-50 border rounded-lg transition ease-in-out duration-200 ">
                  <GitHubLogin clientId="1471b8a44f6578f65757"
                    onSuccess={onSuccessGithub}
                    valid={true}
                    redirectUri="https://github.com/login"
                    onFailure={onFailure}
                ...
                <div className="w-auto flex items-center p-4 bg-white hover:bg-gray-50 border rounded-lg transition ease-in-out duration-200">
                  ...
                  <ReactLoginMS
                    clientId="87ce4562-2762-40fd-8f50-4fbecf3921bf"
