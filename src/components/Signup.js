...
const Signup = ({history}) => {
  ...
  const handleSubmit = (e) => {
    ...
    axios.post("http://localhost:7000/user/register", { firstName, lastName, email, password, repeatPassword }).then((res) => {
      if (res.data.success) {
          ...
          <div className="p-4 py-16 flex flex-col justify-center bg-blueGray-100 h-full">
            <form className="md:max-w-lg mx-auto" onSubmit={handleSubmit}>
              <label className="block mb-4">
                ...
                <input className="px-4 py-3.5 w-full text-gray-400 font-medium placeholder-gray-400 bg-white outline-none border border-gray-300 rounded-lg focus:ring focus:ring-indigo-300" id="signInInput1-1"
                  type="text"
              ...
              <div className="flex flex-wrap justify-between -m-2 mb-4">
                <div className="w-auto p-2">
                  <div className="flex items-center">
                    <input className="w-4 h-4" id="default-checkbox" type="checkbox" value={check} onChange={(e) => setCheck(e.target.value)} />
